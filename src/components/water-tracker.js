...
export default function WaterTracker({ date, waterGoal = 64 }) {
  ...
  return (
    <div className="bg-[var(--surface)] rounded-3xl border border-[var(--border)] p-5">
      ...
      {/* Progress display */}
      <div className="mb-2">
        <span className="text-[var(--text-secondary)] text-sm">{consumed} / {waterGoal} oz</span>
      </div>
      <div className="bg-[var(--surface-2)] rounded-full h-2 mb-5">
        <div
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {[8, 12, 16, 24].map((amount) => (
          <button
            ...
            className="bg-[var(--surface-2)] hover:bg-[var(--border-2)] text-[var(--text-primary)] text-sm rounded-xl px-3 py-2 transition-colors"
          >
            +{amount} oz
          </button>
        ))}
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          placeholder="oz"
          className="bg-[var(--surface-2)] hover:bg-[var(--border-2)] text-[var(--text-primary)] text-sm rounded-xl px-3 py-2 w-20 outline-none focus:ring-1 focus:ring-blue-400 transition-colors"
        />
        <button
      ...
      {/* Log list */}
      {waterLogs.length === 0 && (
        <p className="text-[var(--text-tertiary)] text-xs text-center py-2">No entries yet</p>
      )}
      {waterLogs.length > 0 && (
        <ul className="space-y-1">
          {waterLogs.map((log) => (
            <li key={log.id} className="flex items-center justify-between">
              <span className="text-[var(--text-secondary)] text-xs">{log.amount} oz</span>
              <button
